...
def goodknight(G, s, t):
    ...
    queue = deque()
    queue.append((0, 0, 0, s))  # delay, time, exhaust, castle
    times = [[inf] * 17 for _ in range(n)]
    times[s][0] = 0

    while queue:
        delay, time, exhaust, castle = queue.popleft()

        if times[castle][exhaust] < time:
            continue
        ...
        for (
            ...
            if new_exhuast > 16:
                ...
                if times[new_castle][dist] > new_time:
                    ...
                    queue.append((dist, new_time, dist, new_castle))

            else:
                new_time = time + dist
                if times[new_castle][new_exhuast] > new_time:
                    times[new_castle][new_exhuast] = new_time
                    queue.append((dist, new_time, new_exhuast, new_castle))
